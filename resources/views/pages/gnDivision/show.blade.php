...
<div class="flex items-center justify-between heading text-xl px-2 py-2 border-b-2 border-gray-400">
  <div class="">
    <span class="text-gray-900 font-semibold">{{ $gnDivision->name }}</span> Grama Niladhari Division
    <span class="text-xs text-gray-800 capitalize">| All the data of {{ $gnDivision->name }} of {{ $gnDivision->divSec->name }} divisional secretariat.</span>
  </div>

  <div class="mr-2 text-sm">
    <a href="{{ route('district.show', $gnDivision->divSec->pollingDivision->district) }}">{{ $gnDivision->divSec->pollingDivision->district->name }}</a> <span class="text-gray-700">DST</span> /
    <a href="{{ route('pollingDivision.show', $gnDivision->divSec->pollingDivision) }}">{{ $gnDivision->divSec->pollingDivision->name }}</a> <span class="text-gray-700">PD</span> /
    <a href="{{ route('divsec.show', $gnDivision->divSec) }}">{{ $gnDivision->divSec->name }}</a> <span class="text-gray-700">DS</span> /
    {{ $gnDivision->name }} <span class="text-gray-700">GND</span>
  </div>
</div>
...
<div class="flex justify-between mx-4 my-4 max-w-md md:max-w-2xl">
  <h2 class="ml-4 text-lg text-gray-800 font-semibold">Villages</h2>
  <a class="text-blue-500 font-semibold hover:text-blue-400 px-2" href="{{ route('village.create', $gnDivision) }}">Add new</a>
</div>
