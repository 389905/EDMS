...
<div class="heading flex justify-between text-xl px-2 py-2 border-b-2 border-gray-300">
  <div class="">
    <span class="text-gray-900">Create New Divisional Secretariat</span>
...
<form class="mx-auto w-full max-w-lg mx-8 my-8 px-4 py-4 bg-white shadow-lg rounded-lg" method="post" action="{{ route('divsec.store') }}">
  @csrf
  ...
  <div class="w-full  mb-6 ">
    ...
    <div class="relative">
      <select name="polling_division_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-role">
        @foreach ($pollingDivisions as $key => $pollingDivision)
          <option class="capitalize" value="{{ $pollingDivision->id }}">{{ $pollingDivision->name }}</option>
        @endforeach
      </select>
